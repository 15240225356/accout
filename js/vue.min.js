...
Vue.filter("amount", function(value) {
    ...
    var str = new Number(value).toFixed(2);
    var splitData = str.split(".");
    var intDat = splitData[0];
    var mod = intDat.length % 3, commaNum = Math.floor(intDat.length / 3);
    var formatStr = "";
    for (var index = commaNum; index >= 0; index--) {
        if (index == commaNum) {
            formatStr = intDat.substring((index-1)*3 + mod, index * 3 + mod) + formatStr;
        } else if (index != 0) {
            formatStr = intDat.substring((index-1) * 3 + mod,index * 3 + mod) + "," + formatStr;
        } else if (mod != 0) {
            formatStr = intDat.substring(0, mod)  + "," + formatStr;
        }
    }
    var formatStr = formatStr + "." + splitData[1];
    return formatStr;
});